...
% - Use \sectionname not Section, \figname not Figure.
...
As a first demonstration, we generate fake radial velocity data (with
...
The eccentricity is deliberately chosen to be high and the period and $\asini$
are chosen to be broadly consistent with the typical sub-stellar companion found
in recent analysis of the \apogee\ data (\citealt{Troup:2016}).
The values of the parameters used for the simulated data (e.g., the truth) are:
$e = 0.867$, $P = 53.09~{\rm day}$, $\asini = 42~R_\odot$, $\omega =
134.834^\circ$, $\phi_0 = 342.257^\circ$, $v_0 = 19.431~\kms$.
We generate $J=2^{28}$ samples from our prior pdf over the non-linear
parameters, of which XXX survive the rejection step.

Figure~\ref

\subsection{Experiment 2: Sampling from a highly multi-model posterior pdf for
a known companion}

We choose an \apogee\ target with a previously
identified companion (2M00110648+6609349) but with few radial velocity
measurements (\citealt{Troup:2016}).
\figname~\ref{fig:exp2-rv} shows radial velocity data for the star (black
...
Over-plotted on \figname~\ref{fig:exp2-rv} as grey lines are orbits computed
...
step.
We draw 512 of these at random and show these in \figname~\ref{fig:exp2-rv}.
From these orbits, it appears that there are at least a few distinct period
modes and a wide variety of eccentricities.

This is also demonstrated in \figname~\ref{fig:exp2-corner}, which shows
...
Starting from the top of \figname~\ref{fig:exp3-rv-0} with the full set of 19
...
\begin{figure*}[p]
\begin{center}
\includegraphics[height=0.9\textheight]{figures/exp3-rv-curves-0.pdf}
\end{center}
\caption{%
...
\begin{figure*}[p]
\begin{center}
\includegraphics[height=0.9\textheight]{figures/exp3-rv-curves-1.pdf}
\end{center}
\caption{%
Same as \figname~\ref{fig:exp3-rv-0} for smaller sub-samplings of the
original data set.
\todoapw{More comments here..}
\label{fig:exp3-rv-1}}
\end{figure*}
...
The structure in \figname~\ref{fig:exp3-corner-1} is much more interesting.
