...
radial-velocity measurements of two-body systems that can produce a posterior
sampling for orbital parameters even when the likelihood function is poorly
behaved.}
% Methods
...
With very informative data, the sampling becomes effectively unimodal but too
sparse and we therefore follow the rejection sampling with standard MCMC.
% Results
...
pdfs, which are still highly informative and can be used in hierarchical
(population) modeling.
We give some examples that show how the posterior probability distribution
depends sensitively on the number and time coverage of the observations and
their uncertainties.
\end{abstract}
...
\resp{Converged, indepdendent samplings are essential to the jobs of delivering
...
\citealt{Brewer:2013,Brewer:2014} for other examples of hierarchical inference
in astronomy)}.
