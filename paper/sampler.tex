...
of planets \citep[for a review, see][]{Udry:2007}.
...
(and trinary) star systems \citep[e.g.][]{Raghavan:2010,Tokovinin:2014,Troup:2016}.
...
targets---that are observed multiple times \citep[e.g.[]{Majewski:2015}
...
Ultimately, we assume that the radial velocity curve of a single-line spectroscopic binary system can be specified by six parameters
(\citealt{Kepler:1609}). Here we adopt a parameterization similar to
\citep{Murray:2010}: $P$, $\phi_0$, $\omega$,
$e$, $\asini$, and $v_0$, which are the period, pericenter phase and argument, eccentricity, projected semi-major axis, and the barycenter velocity. As is well-known, this does not fully specify the binary system itself, because of
the $\sin{i}$ and mass function degeneracies.
To model this, we assume that
...
\item the noise contributions to individual
...
The radial velocity $v$ at time\footnote{In our implementation, we subtract off
the median time from our data before starting so this phase actually corresponds
to the time of pericenter passage relative to the median time.} $t$ is then
given by (see also equation~63 in \citealt{Murray:2010})
\begin{equation}
...
of these six parameters, accounting for the fact that the support of this pdf
...
\todo{HWR: is that referenceable? Or is it just lore? I am actually more
familiar with the term `rejection sampling'}
...
Next, we have to choose a number of samples that samples the prior sufficiently
densely.
In practice, we are usually required to take $J=2^{28}$ samples in the prior-pdf
sampling; indeed, a quarter billion samples. % Holy fuck that's a lot!
...
\item $M<M_{\rm min}$ samples survive the rejection, and these samples
  ...
  smaller than $\Delta$; i.e. they give no indication of period ambiguity.
  In that case we assume that the posterior {\it pdf} is unimodal, and
  we use the surviving samples (or sample) to
...
\item $M<M_{\rm min}$ samples survive the rejection, and these samples
  ...
  larger than $M_{\rm min}$. This is expensive!\todo{~HWR: What exactly does `iterate' mean here? What do you do operationally? How many samples, over which span?..}
\end{itemize}
