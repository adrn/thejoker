...
def write_table_hdf5(
    ...
    **create_dataset_kwargs
    ...
    if name in output_group:
        if append and overwrite:
            ...
            if meta_path(name) not in output_group:
                raise ValueError(
                    "No metadata exists for existing table. We "
    ...
    if existing_header is None:  # Just write the table and metadata
        ...
        if not _custom_tbl_dtype_compare(
            existing_header["datatype"], this_header["datatype"]
        ):
            raise ValueError(
                "Cannot append table to existing file because "
...
def inferencedata_to_samples(joker_prior, inferencedata, data, prune_divergences=True):
    """
    ...
    data, *_ = validate_prepare_data(
        data, joker_prior.poly_trend, joker_prior.n_offsets
    )

    samples = JokerSamples(
        poly_trend=joker_prior.poly_trend,
...
def trace_to_samples(self, trace, data, names=None):
    """
    Create a ``JokerSamples`` instance from a pymc trace object.
    ...
    data, *_ = validate_prepare_data(data, self.prior.poly_trend, self.prior.n_offsets)

    samples = JokerSamples(
        poly_trend=self.prior.poly_trend,
