...
def get_t_grid(data, P, span_factor=0.1, max_t_grid=None):
    ...
    t_grid = np.arange(
        data.t.mjd.min() - w * span_factor / 2,
...
@deprecated_renamed_argument(
    "relative_to_t0", "relative_to_t_ref", since="v1.2", warning_type=DeprecationWarning
)
def plot_rv_curves(
    samples,
    t_grid=None,
    ...
    if data is not None:
        data, ids, _ = validate_prepare_data(
            data, samples.poly_trend, samples.n_offsets
        )

    if t_grid is None:
        if data is None:
            raise ValueError(
                "If data is not passed in, you must specify " "the time grid."
            )
    ...
    model_ylim = (
        np.percentile(model_rv.min(axis=1), 5),
    ...
    if data is not None:
        if apply_mean_v0_offset:
            ...
            for i, name in enumerate(dv0_names):
                ...
                data_err[mask] = np.sqrt(data_err[mask] ** 2 + np.var(offset_samples))
            data = RVData(
                t=data.t, rv=data_rv * data.rv.unit, rv_err=data_err * data.rv.unit
            )
        ...
        data.plot(
            ax=ax, relative_to_t_ref=relative_to_t_ref, add_labels=False, **data_style
        )
    ...
    if data_ylim is not None:
        ylim = (min(data_ylim[0], model_ylim[0]), max(data_ylim[1], model_ylim[1]))
...
def plot_phase_fold(
    ...
    n_phase_samples=4096,
    add_labels=True,
    show_s_errorbar=True,
    residual=False,
    remove_trend=True,
    plot_kwargs=None,
    ...
    Parameters
    ----------
    sample : :class:`~thejoker.sampler.JokerSamples`
        One posterior sample from The Joker.
    data : `~thejoker.data.RVData`, optional
        Over-plot the data as well.
    ax : `~matplotlib.Axes`, optional
        A matplotlib axes object to plot on to. If not specified, will
    ...
    if len(sample) > 1:
        msg = "You must pass in a single sample"
        raise ValueError(msg)

    if data is not None:
        data, ids, _ = validate_prepare_data(data, sample.poly_trend, sample.n_offsets)
    ...
    orbit_style.setdefault("alpha", 1.0)
    ...
    if data is not None:
        ...
        if remove_trend:
            # HACK:
            trend = orbit._vtrend
            orbit._vtrend = lambda t: 0.0
            rv = rv - trend(data.t)

        v0_offset_names = get_v0_offsets_equiv_units(sample.n_offsets).keys()
        for i, offset_name in zip(range(1, sample.n_offsets + 1), v0_offset_names):
        ...
        ax.errorbar(
            phase, rv.to(rv_unit).value, data.rv_err.to(rv_unit).value, **data_style
        )
        ...
        if show_s_errorbar and "s" in sample.par_names:
            ax.errorbar(
                phase,
                rv.to(rv_unit).value,
                np.sqrt(data.rv_err**2 + sample["s"] ** 2).to(rv_unit).value,
                linestyle="none",
                marker="",
                elinewidth=0.0,
                color="#aaaaaa",
    ...
    if not residual:
        ax.plot(
            phase_grid,
            orbit.radial_velocity(t0 + P * unit_phase_grid).to_value(rv_unit),
            **orbit_style,
        )
    ...
    if add_labels:
        if with_time_unit is not False:
            ax.set_xlabel(
                r"phase, $(t-t_0)~{\rm mod}~P$ " + f"[{time_unit:latex_inline}]"
            )
        else:
